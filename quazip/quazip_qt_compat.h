...
inline const char* decoding(const QByteArray& str, QStringConverter::Encoding coding)
{
    auto to  = QStringDecoder(coding);
    auto data = to.decode(str);
    return data.data.constData();
}
