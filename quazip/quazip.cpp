...
class QuaZipPrivate {
  ...
  private:
    ...
    //QStringConverter::Encoding fileNameEncoding;
    QStringConverter::Encoding fileNameEncoding;
    /// The codec for comments (used when UTF-8 is not enabled).
    //QStringConverter::Encoding commentEncoding;
    QStringConverter::Encoding commentEncoding;
    ...
    inline QuaZipPrivate(QuaZip *q):
      q(q),
      fileNameEncoding(QStringConverter::System),
      commentEncoding(QStringConverter::System),
      ioDevice(nullptr),
    ...
    /// Stores map of filenames and file locations for unzipping
      ...
      inline static QStringConverter::Encoding defaultFileNameEncoding{ QStringConverter::Encoding::System };
};
  ...
  switch(p->mode) {
    ...
    case mdAdd:
      p->zipError=zipClose(p->zipFile_f, p->comment.isNull() ? nullptr : isUtf8Enabled()
        ...
        : decoding(p->comment.toUtf8(),p->commentEncoding));
      break;
    default:
...
QString QuaZip::getComment()const
{
  ...
  return (unzGetFileFlags(p->unzFile_f, &flags) == UNZ_OK) && (flags & UNZ_ENCODING_UTF8)
    ? QString::fromUtf8(comment) :decoding(comment,p->commentEncoding);
}
...
bool QuaZip::getCurrentFileInfo(QuaZipFileInfo64 *info)const
{
  ...
  info->name=(info->flags & UNZ_ENCODING_UTF8) ? QString::fromUtf8(fileName) : decoding(fileName,p->fileNameEncoding);
  info->comment=(info->flags & UNZ_ENCODING_UTF8) ? QString::fromUtf8(comment) :decoding(comment,p->commentEncoding);
  info->extra=extra;
...
QString QuaZip::getCurrentFileName()const
{
  ...
  QString result = (file_info.flag & UNZ_ENCODING_UTF8)
    ? QString::fromUtf8(fileName) : decoding(fileName,p->fileNameEncoding);
  if (result.isEmpty())
      return result;
  // Add to directory map
...
void QuaZip::setFileNameEncoding(QStringConverter::Encoding fileNameEncoding)
{
  p->fileNameEncoding=fileNameEncoding;
}

void QuaZip::setFileNameEncoding(const char *fileNameEncodingName)
{
    p->fileNameEncoding=QStringConverter::encodingForName(fileNameEncodingName).value_or(QStringConverter::System);
}
...
void QuaZip::setCommentEncoding(QStringConverter::Encoding commentEncoding)
{
  p->commentEncoding=commentEncoding;
}

void QuaZip::setCommentEncoding(const char *commentEncodingName)
{
    p->commentEncoding= QStringConverter::encodingForName(commentEncodingName).value_or(QStringConverter::System);
}
...
void QuaZip::setDefaultFileNameEncoding(QStringConverter::Encoding codec)
{
    QuaZipPrivate::defaultFileNameEncoding = codec;
}

void QuaZip::setDefaultFileNameEncoding(const char *codecName)
{
    setDefaultFileNameEncoding(QStringConverter::encodingForName(codecName).value_or(QStringConverter::System));
}
