...
void TestJlCompress::extractFile()
{
    ...
    if (!createTestArchive(zipName, fileNames,
                           QTextEncoding::codecForName(encoding))) {
        QFAIL("Can't create test archive");
    }
    QuaZip::setDefaultFileNameEncoding(encoding);
...
void TestJlCompress::extractDir_data()
{
    ...
    QTest::addColumn<QByteArray>("fileNameEncodingName");
...
void TestJlCompress::extractDir()
{
    ...
    QFETCH(QByteArray, fileNameEncodingName);
    ...
    QStringConverter::Encoding fileNameEncoding = NULL;
    if (!fileNameEncodingName.isEmpty())
        fileNameEncoding = QTextEncoding::codecForName(fileNameEncodingName);
    QDir curDir;
    ...
    if (!createTestArchive(zipName, fileNames, fileNameEncoding)) {
        QFAIL("Couldn't create test archive");
    }
    ...
    if (fileNameEncoding == NULL)
        ...
        extracted = JlCompress::extractDir(zipName, fileNameEncoding, extDir);
    ...
    if (fileNameEncoding == NULL)
        ...
        extracted = JlCompress::extractDir(&zipFile, fileNameEncoding, extDir);
    QCOMPARE(extracted.count(), expectedExtracted.count());
    foreach (QString fileName, expectedExtracted) {
        QString fullName = dir + fileName;
        QFileInfo fileInfo(fullName);
