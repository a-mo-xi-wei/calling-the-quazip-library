...
void TestQuaZip::setFileNameEncoding_data()
{
    QTest::addColumn<QString>("zipName");
    QTest::addColumn<QStringList>("fileNames");
...
void TestQuaZip::setFileNameEncoding()
{
    QFETCH(QString, zipName);
    QFETCH(QStringList, fileNames);
    ...
    testZip.close();
    testZip.setFileNameEncoding(encoding);
    QVERIFY(testZip.open(QuaZip::mdUnzip));
...
void TestQuaZip::setCommentEncoding()
{
    QuaZip zip("commentEncoding.zip");
    QVERIFY(zip.open(QuaZip::mdCreate));
    zip.setCommentEncoding("WINDOWS-1251");
    ...
    zip.setCommentEncoding(QTextEncoding::codecForName("KOI8-R"));
